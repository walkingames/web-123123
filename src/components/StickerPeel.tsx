...
const StickerPeel: React.FC<StickerPeelProps> = ({
  ...
  useEffect(() => {
    ...
    const computedBounds = { top: 80, left: 0, right: window.innerWidth - 120, bottom: window.innerHeight - 40, ...bounds };
    ...
    const handleResize = () => {
      ...
      const b = computedBounds;
      const newX = Math.max(b.left, Math.min(currentX, b.right));
      const newY = Math.max(b.top, Math.min(currentY, b.bottom));
